...
const ProfileE = ({ navigation }) => {
  ...
  const isValidForm = () => {
    ...
    //Ingresar nuevos nombres
    if (!nombres.trim() || nombres.length < 3)
      return updateError("Nombre muy corto", setError);
    //Ingresar nuevo apellidos
    if (!apellidos.trim() || apellidos.length < 3)
      return updateError("Apellido muy corto", setError);
    //Ingresar nuevo email
    if (!isEmailValid(correo)) return updateError("Email invalido", setError);
    //Ingresar nueva contraseña
    if (!contraseña.trim() || contraseña.length < 3)
      return updateError("Contraseña debe tener 8 caracteres", setError);
    //Ingresar nuevo numero de tarjeta (Aun no se si sirve XD)
    if (!numeroTarjeta.trim() || numeroTarjeta.length <= 9)
      return updateError("Numero de tarjeta corto", setError);
            ...
            <View style={styles.containerForm}>
              ...
              <View style={styles.inputProfile}>
                <TextInput
                  value={apellidos}
                  onChangeText={(value) =>
                    handleOnChangeText(value, "apellidos")
                  }
                  style={styles.inputProfileE}
              ...
              <TouchableOpacity onPress={submitForm} style={styles.buttonSave}>
